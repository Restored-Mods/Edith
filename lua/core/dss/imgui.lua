...
if not ImGui.ElementExists("EdithRestored") then
    ImGui.AddElement("tcMods", "EdithRestored", ImGuiElement.Menu, "Edith")
end
...
for _, collectible in pairs(orderedItems) do
    ...
    ImGui.AddCallback(elemName, ImGuiCallback.Render, function()
            local val = true
            for indexItem, disabledItem in ipairs(TSIL.SaveManager.GetPersistentVariable(EdithRestored, "DisabledItems")) do
                if disabledItem == GetItemsEnum(collectible.ID) then
                    val = false
                    break
                end
...
ImGui.AddCallback("edithWindowSettings", ImGuiCallback.Render, function()
    ImGui.UpdateData("edithPushToSlide", ImGuiData.Value, TSIL.SaveManager.GetPersistentVariable(EdithRestored, "AllowHolding") == 1)
    ImGui.UpdateData("edithAllowBombs", ImGuiData.Value, TSIL.SaveManager.GetPersistentVariable(EdithRestored, "OnlyStomps") == 1)
    ...
    ImGui.UpdateData("edithAlwaysShowMoonPhase", ImGuiData.Value, TSIL.SaveManager.GetPersitentVariable(EdithRestored, "AlwaysShowMoonPhase") == 1)
end)
...
for key, val in pairs(unlocksMarksA) do
	...
	ImGui.AddCombobox("edithUnlocksA", "edithMark"..key, val, function(index, val)
		...
		if index > 0 then
			if not Isaac.GetPersistentGameData():Unlocked(EdithRestored.Enums.Achievements.Marks.ASide[key]) then
				Isaac.ExecuteCommand("achievement "..marksA[key])
			end
            if Isaac.AllMarksFilled(EdithRestored.Enums.PlayerType.EDITH) and not Isaac.GetPersistentGameData():Unlocked(EdithRestored.Enums.Achievements.CompletionMarks.LOT_BABY) then
                Isaac.ExecuteCommand("achievement "..EdithRestored.Enums.Achievements.CompletionMarks.LOT_BABY)
            end
		end
	end, { 'None', 'Normal', 'Hard' }, 0, true)
	ImGui.AddCallback("edithMark"..key, ImGuiCallback.Render, function()
        ImGui.UpdateData("edithMark"..key, ImGuiData.Value, Isaac.GetCompletionMark(EdithRestored.Enums.PlayerType.EDITH, key))
	end)
end

ImGui.AddCombobox("edithUnlocksA", "edithMarkAll", "All Marks", function(index, val)
	if index < 1 then
		Isaac.ExecuteCommand("lockachievement "..EdithRestored.Enums.Achievements.CompletionMarks.LOT_BABY)
	elseif not Isaac.GetPersistentGameData():Unlocked(EdithRestored.Enums.Achievements.CompletionMarks.LOT_BABY) then
		Isaac.ExecuteCommand("achievement "..EdithRestored.Enums.Achievements.CompletionMarks.LOT_BABY)
	end
end, { 'None', 'Unlocked' }, 0, true)

ImGui.AddCallback("edithMarkAll", ImGuiCallback.Render, function()
    local unlocked = Isaac.GetPersistentGameData():Unlocked(EdithRestored.Enums.Achievements.CompletionMarks.LOT_BABY)
