...
function KnockoutDrops:OnStompModify(player, stompDamage, radius, knockback, doBombStomp, isStompPool)
	...
	if rng:RandomFloat() <= maxchance or isStompPool.PoolFruitCake or isStompPool.PoolFFEmojiGlases then
...
EdithRestored:AddPriorityCallback(
	...
	{ Item = CollectibleType.COLLECTIBLE_KNOCKOUT_DROPS, PoolFruitCake = true, PoolFFEmojiGlases = true }
)
...
function KnockoutDrops:OnStomp(player, stompDamage, bombLanding, forced, isStompPool)
    if isStompPool.PoolFruitCake or isStompPool.PoolFFEmojiGlases or forced then
